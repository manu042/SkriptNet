...
;(function ($) {
	...
	tsp = ts.pager = {
		...
		init: function (table) {
			...
			if (table.hasInitialized && table.config.pager && table.config.pager.initialized) {
				return;
			}
			var t,
				c = table.config,
		...
		updatePageDisplay: function (c, completed) {
			if (c.pager && c.pager.initializing) {
				return;
			}
		...
		moveToPage: function (c, p, pageMoved) {
			...
			if (p.ajax && !wo.filter_initialized && ts.hasWidget(table, 'filter')) {
				return;
			}
