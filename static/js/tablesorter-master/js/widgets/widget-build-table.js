...
;(function ($) {
	...
	var ts = $.tablesorter = $.tablesorter || {},
		...
		bt = ts.buildTable = function (tar, c) {
			...
			if (!ts.buildTable.hasOwnProperty(typ) && typ !== '') {
				if (c.debug) {
					console.error('aborting build table widget, incorrect build type');
				}
				return false;
			}
			...
			if (d instanceof jQuery) {
				...
				$.ajax(wo.build_source)
					...
					.fail(function (jqXHR, textStatus, errorThrown) {
						if (c.debug) {
							console.error('aborting build table widget, failed ajax load');
						}
	...
	bt.object = function (table, data, wo) {
		...
		if (!h || !r || h.length === 0 || r.length === 0) {
			if (c.debug) {
				console.error('aborting build table widget, missing data for object build');
			}
			return false;
		}
