		...
		tsff = ts.filterFormatter = $.extend({}, ts.filterFormatter, {
			...
			uiSpinner: function ($cell, indx, spinnerDef) {
				...
				o.create = function (event, ui) {
					...
					if (typeof o.oldcreate === 'function') {
						o.oldcreate(event, ui);
					}
				};
				o.spin = function (event, ui) {
					updateSpinner(ui);
					if (typeof o.oldspin === 'function') {
						o.oldspin(event, ui);
					}
				};
				if (o.addToggle) {
			...
			uiDateCompare: function ($cell, indx, defDate) {
				...
				o.onClose = function (selectedDate, ui) {
					date1Compare();
					if (typeof o.oldonClose === 'function') {
						o.oldonClose(selectedDate, ui);
					}
				};
				$date.datepicker(o);
